...
<form action="book-search1.php" method="POST">
...
if ($db->connect_error) {
    ...
    printf("<br><a href=NjeriOlenkereLab1.php>Return to home page </a>");
    exit();
}

# Build the query. Users are allowed to search on title, author, or both

$query = " select * bookid, author, title, onloan, borrowerid";
if ($searchtitle && !$searchauthor) { // Title search only
    $query = $query . " where title like '%" . $searchtitle . "%'"; //percenage means any character coming b4 or after
}
...
# Here's the query using bound result parameters
    ...
    $stmt->bind_result($bookid, $title, $author, $onloan, $duedate, $borrowerid);//whatever you is assigned in queery and prepares it for execution
    $stmt->execute();
